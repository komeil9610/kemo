...
const DEFAULT_EXCEL_FILE = path.resolve(__dirname, '../../data/data.xlsx');
const EXTRACT_SCRIPT = path.resolve(__dirname, '../../scripts/extract_excel_orders.py');
...
const normalizeImportedRow = (sheetName, row) => {
  ...
  if (!city || !district || !preferred.date || !acDetails.length) {
    return null;
  }
...
const parseExcelWorkbook = async (filePath = DEFAULT_EXCEL_FILE) => {
  const { stdout } = await execFileAsync('python3', [EXTRACT_SCRIPT, filePath], {
